 ...
 public GlobaleDaten selectVerband(){
    ...
    public GlobaleDaten selectSani(){
        ...
        if(cursor.getCount() >= 1){
            ...
            Log.d(LOG_TAG, mfall.toString()+ " SAni");
        }
        return mfall;
    }
    ...
    */
    ...
    private GlobaleDaten cursorToUebersicht(Cursor cursor) {

        //Es werden zunächst die Position der Fall-ID und des Patientennamen übergeben.
        int idIndex = cursor.getColumnIndex(FalleingabeContract.Tbl_Einsatz.COL_ID);
        int nameIndex = cursor.getColumnIndex(FalleingabeContract.Tbl_Patient.COL_NAME);
        int vornameIndex = cursor.getColumnIndex(FalleingabeContract.Tbl_Patient.COL_VORNAME);

        //Hier wird der Inhalt der Zeile extrahiert.
        int id = cursor.getInt(idIndex);
        String name = cursor.getString(nameIndex);
        String vorname = cursor.getString(vornameIndex);

        //laut Tutorial soll es ...new GlobaleDaten(id, name, vorname); heißen
        GlobaleDaten GlobaleDaten = new GlobaleDaten();
        GlobaleDaten.setPat_name(name);
        GlobaleDaten.setPat_vorname(vorname);
        GlobaleDaten.setEin_ID(id);

        return GlobaleDaten;
    }
public GlobaleDaten selectPatient(){
    //query(String table, String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy, String limit)
    String[] columns={FalleingabeContract.Tbl_Patient.COL_ID, FalleingabeContract.Tbl_Patient.COL_NAME, FalleingabeContract.Tbl_Patient.COL_VORNAME};
    Cursor cursor=database.query(FalleingabeContract.Tbl_Patient.TABLE_NAME, columns, null, null, null, null, null, null);

    GlobaleDaten mfall=new GlobaleDaten();
    if(cursor.getCount() >= 1){
        cursor.moveToFirst();
        mfall.setPat_name(cursor.getString(1));
        mfall.setPat_vorname(cursor.getString(2));
        Log.d(LOG_TAG, mfall.toString());
    }
    return mfall;
}

    public List<GlobaleDaten> getUebersicht() {
        List<GlobaleDaten> UebersichtList = new ArrayList<>();

        //mir ist nicht ganz klar warum hier erneut Daten aufgerufen werden. Wir brauchen noch die zweite Tabelle für die Prot-ID.
        Cursor cursor = database.query(FalleingabeContract.Tbl_Patient.TABLE_NAME,
                new String[]{FalleingabeContract.Tbl_Patient.COL_NAME, FalleingabeContract.Tbl_Patient.COL_VORNAME},
                null, null, null, null, null);
        cursor.moveToFirst();
        GlobaleDaten GlobaleDaten;

        while (!cursor.isAfterLast()) {
            GlobaleDaten = cursorToUebersicht(cursor);
            UebersichtList.add(GlobaleDaten);
        ...
        return UebersichtList;
    }
   /* public List<String> getUebersichtString() {
        List<String> uebersichtListString = new ArrayList<>();
        Integer pat_id;
        String uebersicht;
        String name;
        String vname;
        //mir ist nicht ganz klar warum hier erneut Daten aufgerufen werden. Wir brauchen noch die zweite Tabelle für die Prot-ID.
        Cursor cursor = database.query(FalleingabeContract.Tbl_Patient.TABLE_NAME,
                new String[]{FalleingabeContract.Tbl_Patient.COL_ID, FalleingabeContract.Tbl_Patient.COL_NAME, FalleingabeContract.Tbl_Patient.COL_VORNAME}, null, null, null, null, null);
        if(cursor.getCount() >= 1) {
            cursor.moveToFirst();
            pat_id = (cursor.getInt(0));
            name = (cursor.getString(1));
            vname = (cursor.getString(2));
        }
        Cursor cursor_ein = database.query(FalleingabeContract.Tbl_Einsatz.TABLE_NAME, new String[]{FalleingabeContract.Tbl_Einsatz.COL_ID},
                FalleingabeContract.Tbl_Einsatz.COL_PAT_ID + "=" + pat_id, null);
        if(cursor_ein.getCount() >= 1) {
            cursor_ein.moveToFirst();
        }
        cursor.moveToFirst();
        GlobaleDaten GlobaleDaten;
        if(cursor.getCount() >= 1) {
            ...
            while (!cursor.isAfterLast()) {
                //GlobaleDaten = cursorToUebersicht(cursor);
                String id = cursor_ein.getString(0);
                uebersicht = pat_id + " " + name + ", " + vname;
                uebersichtListString.add(uebersicht);
                //Log.d(LOG_TAG, "ID: " + shoppingMemo.getId() + ", Inhalt: " + shoppingMemo.toString());
                cursor.moveToNext();
            }
        }
        ...
        return uebersichtListString;
    }
    */
    public GlobaleDaten selectEinsatz(){
        //query(String table, String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy, String limit)

        Cursor cursor=database.query(FalleingabeContract.Tbl_Einsatz.TABLE_NAME, null, null, null, null, null, null, null);
        GlobaleDaten mfall=new GlobaleDaten();
        if(cursor.getCount() >= 1){
            cursor.moveToFirst();
            mfall.setVer_name(cursor.getString(0));
    ...
    public String[] getAllPat() {
        ...
        //Cursor cursor = database.rawQuery("SELECT  * FROM " + FalleingabeContract.Tbl_Patient.TABLE_NAME, null);
        Cursor cursor = database.query(FalleingabeContract.Tbl_Patient.TABLE_NAME,
        ...
        */
        ...
        while (j < patList.size()) {
            ...
            patListString[j]=(patList.get(j).getPat_name())+", "+(patList.get(j).getPat_vorname());
            j++;
        }
